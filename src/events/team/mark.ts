        ...
        if (button[0] === 'pirate') {
            ...
            if (button[1] === 'mark') {
                interaction.guild?.members.fetch(button[2]).then((member) => {
                    const role = db.query(`SELECT value FROM settings WHERE key = 'setting_markedMemberRole'`).get() as { value: string }

                    const ValuateRole = interaction.guild?.roles.cache.get(role.value) as Role
            ...
            if (button[1] === 'ignore') {
                ...
                interaction.reply({
                    content: 'Successfully Ignored Marked member!\nMarked member: <@' + button[2] + '>',
