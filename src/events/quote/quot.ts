...
export default event('messageCreate', async ({ client }, Message) => {
    try {
        ...
        if (currentGuild?.id === messageContent?.[5] && currentGuild?.channels.cache.has(messageContent?.[6]!)) {
            const channel = currentGuild.channels.cache.get(messageContent?.[6]!) as GuildBasedChannel as TextChannel
            ...
            if (message.content) {
                ...
                if (message.attachments.size > 0) {
                    quotedMessage.setImage(Array.from(message.attachments.values())[0].url)
            ...
            } else if (message.embeds.length === 0 && !message.content && message.attachments.size > 0) {
                ...
                const quotedMessage = new EmbedBuilder()
                    ...
                    .setDescription(message.attachments.map((attachment) => `[${attachment.name}](${attachment.url})`).join('\n'))
                    .setFooter({
                        text: `Quoted by ${Message.author.tag} from #${channel.name}`,
                        iconURL: Message.author.displayAvatarURL(),
                    })
