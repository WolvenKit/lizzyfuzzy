...
const meta = new ContextMenuCommandBuilder().setName('Omit User as Pirate').setType(ApplicationCommandType.User)
...
export default command(meta, async ({ interaction }) => {
    ...
    if (!getUser) {
        const setting_markedRole = settingsDB.query(`SELECT value FROM settings WHERE key = 'setting_markedMemberRole'`).get() as { value: string }
